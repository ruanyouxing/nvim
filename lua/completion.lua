...
if vim.fn.has("nvim-0.5.1") == 1 then
	...
	handlers[references] = locations.references_handler
	handlers[definition] = locations.definition_handler
	handlers[declaration] = locations.declaration_handler
	handlers[typeDefinition] = locations.typeDefinition_handler
	handlers[implementation] = locations.implementation_handler
	handlers[documentSymbol] = symbols.document_handler
	handlers[symbol] = symbols.workspace_handler
else
	local bufnr = vim.api.nvim_buf_get_number(0)

	handlers[codeaction] = function(_, _, actions)
		codeAction.code_action_handler(nil, actions, nil, nil, nil)
	end

	handlers[references] = function(_, _, result)
		locations.references_handler(nil, result, {
			bufnr = bufnr,
		}, nil)
	end

	handlers[definition] = function(_, method, result)
	...
	handlers[declaration] = function(_, method, result)
	...
	handlers[typeDefinition] = function(_, method, result)
	...
	handlers[implementation] = function(_, method, result)
	...
	handlers[documentSymbol] = function(_, _, result, _, bufn)
